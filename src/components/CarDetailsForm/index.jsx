...
import { basicData, featuresData, specificationsFields } from "./constants";
...
const CarDetailForm = ({ carId = null, handleClose = () => { } }) => {
    ...
    useEffect(() => {
        if (carDetails?.id) {
            const { thumbnail, features, specifications, images, ...basicData } =
                carDetails;
    ...
    const handleBasicDetailChange = (e, isNumber = false, fieldName = "") => {
        ...
        setBasicDetails((prev) => ({
            ...prev,
            [name || fieldName]: isNumber ? Number(value) : value,
        }));
    };
    ...
    return (
        <div className="p-4 h-[90vh] w-[90vw] sm:w-[80vw] mx-auto bg-white rounded shadow relative">
            {loadingDetails ? (
                <p className="w-full text-center mt-20 text-xl font-medium">Loading car details...</p>
            ) : (
                <>
                    <div className="pb-10 max-h-full overflow-y-auto">
                                    ...
                                    <div key={name} className="flex flex-col">
                                        {isDropdown ? (
                                            <Dropdown
                                                ...
                                                onChange={(e) =>
                                                    handleBasicDetailChange(e, isNumber, name)
                                                }
                                                isMandatory={true}
                                            />
                                        ) : (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                            {["interior", "exterior", "tyres"].map((type) => (
                                <div key={type} className="flex flex-col">
                                    ...
                                    <ImageInput
                                        label={`${images?.[type]?.length || imageUrls?.[type]?.length
                                            ? "Update"
                                            : "Select"
                                            } ${type} images`}
                                        id={`${type}ImageInput`}
                        ...
                        <div>
                            ...
                            {Object.entries(specificationsFields).map(
                                ([sectionKey, fields]) => (
                                    <div
                                        key={sectionKey}
                                        className="border rounded-lg p-4 shadow my-2"
                                    >
                    ...
                    <div className="w-full px-4 py-2 border-t border-gray-400 absolute bottom-0 left-0 right-0 bg-white flex items-center justify-between">
                        {missingBasicDetails?.length > 0 && (
                            <p className="text-red-600">
                                *Missing fields: {missingBasicDetails?.join(", ")}
                            </p>
                        )}
                        <button
                            onClick={handleSubmit}
                            className={`bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 min-w-28 cursor-pointer ml-auto ${isUploading ? "opacity-50 cursor-not-allowed" : ""
                                }`}
